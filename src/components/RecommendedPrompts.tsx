...
function RecommendedPromptCard({ prompt, isEn }: RecommendedPromptCardProps) {
  ...
  // 获取第一张输出图片作为预览
  const previewImage = prompt.outputImages && prompt.outputImages.length > 0 ? prompt.outputImages[0] : null;

  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 hover:shadow-md transition-all group overflow-hidden">
      {/* 图片预览部分 */}
      {previewImage && (
        <div className="relative h-48 overflow-hidden">
          ...
          {/* 图片数量指示器 */}
          {prompt.outputImages && prompt.outputImages.length > 1 && (
            <div className="absolute bottom-2 right-2 bg-black bg-opacity-50 text-white text-xs px-2 py-1 rounded">
              +{prompt.outputImages.length - 1}
            </div>
          )}
        </div>
      )}
      ...
      <div className="p-4">
        <div className="mb-3">
          <div className="flex justify-between items-start mb-2">
            ...
            {!previewImage && <CopyButton text={prompt.prompt} size="sm" />}
          </div>
        ...
        <div className="mb-4">
          ...
          {prompt.guidance && (
            <p className="text-xs text-gray-500 italic line-clamp-2">
              {isEn ? 'Guidance: ' : '指导: '}{truncateText(prompt.guidance, 60)}
            </p>
          )}
        </div>
